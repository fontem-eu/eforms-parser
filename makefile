...
IMAGE   := contribute.void42.internal/golden/eforms-parser
PROJECT := eforms-parser
SRC     := src
TESTS   := tests
...
analyze: test
	$(SCANNER) \
		...
		'-Dsonar.coverage.exclusions=src/eforms/cli.py'
...
sbom:
	cyclonedx-py requirements -r requirements.txt --output-format json -o sbom.json
